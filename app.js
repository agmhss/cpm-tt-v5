 ...
 * Version: The Master Build V6.0 (Super Bypass & 2000x Solver)
 */
...
function getIndividualClasses(classNameStr) {
    ...
    groups.forEach(group => {
        ...
        if (parts.length > 1) {
            ...
            parts.forEach(p => {
                p = p.trim();
                if (p.match(/^(\d+|LKG|UKG)/)) {
                    result.push(p);
                } else {
                    let prefix = p.includes('-') ? baseGrade : baseGrade + '-';
                    result.push(prefix + p.replace(/^-/, ''));
                }
            });
        } else {
...
window.generateGrid = function() {
    ...
    if (mode === 'regular' && window.rawAssignmentsData) {
        ...
        window.runCalculationEngine(true, true); 
        return;
    }
...
// =========================================================
// 🌟 THE CORE ENGINE (Auto Generation)
// =========================================================
function generateAutoTimetable() {
    ...
    const anPeriodLabels = teachingPeriods.slice(4).map(p => p.label);

    const MAX_ITERATIONS = 2000; // 🌟 2000x Iterations for perfect fit
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        ...
        function getMaxFnAn(req) {
            if (req.isClassTeacher) {
                return { fn: 5, an: Math.max(0, req.periodsPerWeek - 5) };
            }
            
            let sub = abbreviateSubject(req.subjectName).toUpperCase();
            let isLangSoc = sub.includes('TAM') || sub.includes('ENG') || sub.includes('SOC') || sub.includes('HIS');
            let isMat = sub.includes('MAT');
            let isSci = sub.includes('SCI');

            if (isLangSoc) return { fn: 3, an: 3 };
            if (isMat || isSci) return { fn: 4, an: 3 };
        ...
        let placeSlot = (day, p, req) => {
            ...
            if (fnPeriodLabels.includes(p.label)) {
                fnSubjectCount[key] = (fnSubjectCount[key] || 0) + 1;
            } else {
                anSubjectCount[key] = (anSubjectCount[key] || 0) + 1;
            }
        ...
        // PHASE 1: CLASS TEACHERS MUST GET PERIOD 1 ON ALL 5 DAYS
        currentAssignments.filter(req => req.isClassTeacher).forEach(req => {
            ...
            daysOfWeek.forEach(day => {
                if (req.assignedCount < req.periodsPerWeek) {
                    ...
                    let ptAvail = isPartTimeTeacherAvailable(req.teacherName, 'FN');

                    if (!isTeacherBusy && !isClassBusy && ptAvail) {
                        placeSlot(day, firstPeriod, req);
                    }
                }
            });
        });

        // PHASE 2: CONTINUOUS LAST TWO PERIODS
        currentAssignments.forEach(req => {
            let sub = req.subjectName.toUpperCase();
            let isDoubleSubject = sub.includes('SCI') || sub.includes('PET') || sub.includes('CE') || sub.includes('CC') || sub.includes('EC');
            let periodsLeft = req.periodsPerWeek - req.assignedCount;

            if (isDoubleSubject && !req.hasDouble && periodsLeft >= 2) {
                let p1 = teachingPeriods[6];
                let p2 = teachingPeriods[7];
                ...
                for (let day of daysToTry) {
                    ...
                    let ptAvail1 = isPartTimeTeacherAvailable(req.teacherName, 'AN');
                    let ptAvail2 = isPartTimeTeacherAvailable(req.teacherName, 'AN');

                    if (!isTeacherBusy && !isClassBusy && ptAvail1 && ptAvail2) {
                        placeSlot(day, p1, req);
                        placeSlot(day, p2, req);
                        req.hasDouble = true;
                        break;
                    }
                }
        ...
        currentAssignments.forEach(req => {
            let sub = req.subjectName.toUpperCase();
            if (sub.includes('CLUB') || sub.includes('JRC') || sub.includes('NSS')) {
                ...
                for (let day of daysToTry) {
                    ...
                    let isClassBusy = indClasses.some(cls => classAvail[cls]?.[timeKey]);
                    let isTeacherBusy = teacherAvail[req.teacherName]?.[timeKey];
                    let ptAvail = isPartTimeTeacherAvailable(req.teacherName, 'AN');

                    if (!isTeacherBusy && !isClassBusy && ptAvail && req.assignedCount < req.periodsPerWeek) {
                        placeSlot(day, p, req);
                        break;
                    }
                }
        ...
        // PHASE 4: MAIN PLACEMENT WITH PRIORITIES
        currentAssignments.sort((a, b) => {
            let gradeA = getGradeValue(a.className);
            let gradeB = getGradeValue(b.className);

            if (iteration === 0) {
                if (gradeA !== gradeB) return gradeB - gradeA;
                return b.periodsPerWeek - a.periodsPerWeek; 
            } 
            let scoreA = gradeA * 10 + b.periodsPerWeek;
            let scoreB = gradeB * 10 + a.periodsPerWeek;
            return (scoreB - scoreA) * Math.random() + (Math.random() - 0.5) * 20;
        });
        ...
        function attemptMainPlacement(req, bypassLevel) {
            ...
            for (let i = 0; i < req.periodsPerWeek; i++) {
                ...
                for (let offset = 0; offset < daysOfWeek.length; offset++) {
                    ...
                    if (bypassLevel < 1 && currentDayCount >= maxDailyAllowed) continue; 

                    let periodsToTry = [...teachingPeriods];
                    if (iteration > 0) periodsToTry.sort(() => Math.random() - 0.5);

                    for (let period of periodsToTry) {
                        // Bypass Level 2 ignores Period 1 restrictions to guarantee 40/40!
                        ...
                        if (bypassLevel < 1 && !isPT) {
                            let curFn = fnSubjectCount[key] || 0;
                            let curAn = anSubjectCount[key] || 0;
                            if (isFN && curFn >= maxLimits.fn) continue;
                            if (!isFN && curAn >= maxLimits.an) continue;
                        }
        ...
        // Pass 1: Ignore FN/AN Balance Limits
        currentAssignments.forEach(req => {
            if (req.assignedCount < req.periodsPerWeek) attemptMainPlacement(req, 1);
        });

        // Pass 2: SUPER BYPASS (Ignore Period 1 rules, just fill the gaps!)
        currentAssignments.forEach(req => {
            if (req.assignedCount < req.periodsPerWeek) attemptMainPlacement(req, 2);
        });
        ...
        if (bestScore === TARGET_SCORE) {
            console.log("Perfect Timetable Achieved at iteration: " + (iteration + 1));
            break;
        }
    }
...
window.handleCellClick = function(day, period) {
    ...
    if (!window.swapSource) {
        ...
        let slot1Index = generatedWeeklyTimetable.findIndex(d => 
            ...
            (viewType === 'teacher' ? d.teacherName === filterVal : getIndividualClasses(d.className).includes(filterVal))
        );
        ...
        teachingPeriods.forEach(period => {
            ...
            if (slot) {
                let cellText = viewType === 'class' 
                    ? `<span class="font-semibold text-gray-800">${abbreviateSubject(slot.subjectName)}</span><br><span class="text-xs text-blue-600 font-bold">${slot.teacherName}</span>`
...
function renderSubstituteSchedule() {
    ...
    let vacantSlots = generatedWeeklyTimetable.filter(slot =>
        slot.day === day && absentTeachers.includes(slot.teacherName)
    );
    ...
    vacantSlots.forEach(slot => {
        ...
        html += `<tr class="border-b hover:bg-gray-50">
            ...
            <td class="p-3 border-r text-red-600 font-medium line-through">${slot.teacherName} <span class="text-xs text-gray-400">(${slot.subjectName})</span></td>
            <td class="p-3">
