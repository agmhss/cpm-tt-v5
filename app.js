 ...
 * Version: The Ultimate Master Build V7.1 (Safe Data Sync & Rule Engine)
 */
...
window.uiAllotments = []; // 🌟 UI-ல் Grouped டேட்டாவைக் காட்ட
...
window.syncFromCloud = async function() {
    try {
        ...
        alert("Sync Failed: Check your SCRIPT_URL -> " + error.message);
    }
};
...
window.runCalculationEngine = function(renderGrid = true, forceRecalculate = false) {
    try {
        ...
        // 🌟 1. Load exact Google Sheet structure into UI Memory
        window.uiAllotments = [];
        window.rawAssignmentsData.slice(1).forEach(row => {
            ...
            window.uiAllotments.push({
                teacherName: tName,
                act1: String(row[2] || '').trim(), cls1: String(row[3] || '').trim(), per1: String(row[4] || '').trim(),
                ct: String(row[5] || '').trim(),
                act2: String(row[6] || '').trim(), cls2: String(row[7] || '').trim(), per2: String(row[8] || '').trim(),
                act3: String(row[9] || '').trim(), cls3: String(row[10] || '').trim(), per3: String(row[11] || '').trim()
            });
        });

        // 🌟 2. Flatten data only for the Algorithm Engine (Does not affect UI)
        ...
        window.uiAllotments.forEach(req => {
            ...
            let classTeacherClass = req.ct.toUpperCase();
            
            let blocks = [
                { act: req.act1, cls: req.cls1, per: req.per1 },
                { act: req.act2, cls: req.cls2, per: req.per2 },
                { act: req.act3, cls: req.cls3, per: req.per3 }
            ];

            blocks.forEach(block => {
                let activity = block.act;
                let classSecStr = block.cls;
                let periodsVal = block.per;
                ...
                distinctClasses.forEach(distinctClassGroup => {
                    ...
                    if (finalPeriods > 0) {
                        let isCT = false;
                        if (classTeacherClass && (cleanGroup.toUpperCase() === classTeacherClass || cleanGroup.toUpperCase().includes(classTeacherClass))) {
                            isCT = true;
                        }
...
window.renderUIAllotmentTable = function() {
    ...
    // 🌟 Renders EXACTLY like your Google Sheet (Grouped by Teacher)
...
window.addTeacherAllotmentUI = function() {
    ...
    window.uiAllotments.push({
        teacherName: tName, ct: isCT ? tClass : "",
        act1: tSub, cls1: tClass, per1: tPeriods,
        act2: "", cls2: "", per2: "",
        act3: "", cls3: "", per3: ""
    });
...
window.saveAllotmentToCloud = async function() {
    ...
    try {
        await fetch(SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ action: "saveAllotment", data: window.uiAllotments })
        });
        alert("Master Allotment Ledger Saved to Cloud securely!");
        updateStatus("System Ready");
    } catch(e) {
...
window.generateGrid = function() {
    const mode = document.getElementById('opMode')?.value || 'regular';
