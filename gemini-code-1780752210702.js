 ...
 * Version: The Ultimate Master Build V8.1 (Exam/Sub Tabs & CT Input Fix)
 */
...
function getIndividualClasses(classNameStr) {
    ...
    groups.forEach(group => {
        ...
        if (parts.length > 1) {
            ...
            if (cleanGroup) result.push(cleanGroup);
        }
    });
    return result;
}
...
function populateAbsentTeachersList() {
    try {
        ...
        if (allTeachers.length === 0) {
            listDiv.innerHTML = `<span class="text-xs text-red-500 font-bold p-2">No teachers found.</span>`;
            return;
        }
...
window.switchAdminTab = function(tabId) {
    ...
    if(tabId === 'tabGenerator') {
        ...
        if(viewType && generatedWeeklyTimetable.length > 0) {
            viewType.value = 'class'; viewType.dispatchEvent(new Event('change'));
        }
    }
};
...
window.runCalculationEngine = function(renderGrid = true, forceRecalculate = false) {
    try {
        ...
        window.teacherWorkload = {}; window.teacherMaxGrade = {}; 
...
window.generateGrid = function() {
    ...
    mainGrid.innerHTML = `
        <div class="flex flex-col items-center justify-center h-full text-blue-600 py-20">
            <svg class="animate-spin h-14 w-14 mb-5 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            <h2 class="text-2xl font-black tracking-widest animate-pulse uppercase">Engine Processing...</h2>
            <p class="text-sm font-bold text-gray-500 mt-2 bg-gray-100 px-4 py-1 rounded-full">Executing 2000x Monte Carlo Iterations</p>
        </div>
    `;
...
function generateAutoTimetable() {
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        let tempTimetable = [];
        let teacherAvail = {}; let classAvail = {}; let dailySubjectCount = {}; let fnSubjectCount = {}; let anSubjectCount = {}; 
        let currentScore = 0;
...
window.addTeacherAllotmentUI = function() {
    ...
    let tCT = document.getElementById('newTCT').value.trim().toUpperCase(); // Changed to Text Input
    ...
    document.getElementById('newTSub').value = ''; document.getElementById('newTClass').value = ''; document.getElementById('newTCT').value = '';
    renderUIAllotmentTable();
};

window.deleteAllotmentRow = function(index) {
    if(confirm("Delete this allotment?")) { window.uiAllotments.splice(index, 1); renderUIAllotmentTable(); }
};

window.saveAllotmentToCloud = async function() {
    updateStatus("Saving Master Ledger...");
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: "saveAllotment", data: window.uiAllotments }) });
        alert("Master Allotment Ledger Saved to Cloud securely!");
        updateStatus("System Ready");
    } catch(e) { alert("Error saving allotment."); }
};
...
window.addDynamicRule = function() {
    ...
    window.dynamicRules.push({ ruleId: 'R' + Date.now(), targetType, targetName, constraintType: constraint, parameter, action });
    renderDynamicRulesTable();
};
...
window.renderDynamicRulesTable = function() {
    ...
    tbody.innerHTML = window.dynamicRules.map((r, index) => `
        <tr class="hover:bg-purple-50 transition-colors">
            <td class="p-3 font-bold text-gray-600">${r.targetType}</td><td class="p-3 text-purple-700 font-black">${r.targetName}</td>
            <td class="p-3 text-sm">${r.constraintType}</td><td class="p-3 font-bold bg-gray-100 rounded text-center">${r.parameter}</td>
...
window.saveRulesToCloud = async function() {
    ...
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: "saveRules", data: window.dynamicRules }) });
        alert("Rules constraints saved successfully!");
        updateStatus("System Ready");
    } catch(err) { alert("Error saving rules."); }
};
...
window.renderExamSchedule = function() {
    ...
    const currentSession = document.getElementById('examSession').value;
    ...
    if (SCHOOL_CONFIG.assignments) {
        SCHOOL_CONFIG.assignments.forEach(req => {
            if (!teacherProfiles[req.teacherName]) teacherProfiles[req.teacherName] = { subjects: new Set() };
            teacherProfiles[req.teacherName].subjects.add(req.subjectName);
        });
    }

    let allTeachers = Object.keys(teacherProfiles);
    let presentTeachers = allTeachers.filter(t => !busyInOtherSession.includes(t) && window.teacherPartTimeStatus[t] === 'FULL' && !t.includes('*') && !t.includes('⭐'));
...
window.renderSubstituteSchedule = function() {
    ...
    const selectedDate = document.getElementById('subDate').value ? new Date(document.getElementById('subDate').value).toLocaleDateString('en-GB') : "N/A";
...
window.saveDutiesToCloud = async function(mode) {
    ...
    if (mode === 'sub') {
        const selects = document.querySelectorAll('#subGrid select'); 
        let finalDutyTracker = { ...window.subDutyTracker }; 
        selects.forEach(select => { if (select.value) finalDutyTracker[select.value] = (finalDutyTracker[select.value] || 0) + 1; });
        ...
        if (assignedTeachers.length === 0) return alert("No teachers assigned. Nothing to save.");
    ...
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: actionType, data: payloadData }) });
        alert(`${mode.toUpperCase()} Duty counts saved!`); updateStatus("System Ready");
...
window.submitTeacherConstraints = async function() {
    ...
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: "saveRules", data: window.dynamicRules }) });
        alert("Your preferences have been locked and submitted to the Master Engine!");
...
window.exportPDF = function(mode) {
    ...
    if (viewType === 'all' || !filterVal) {
        ...
        const cW = 90, cH = 52, marginX = 12, marginY = 12, gapX = 6, gapY = 4; let cardsOnPage = 0;
        const teachingPeriods = SCHOOL_CONFIG.regularTimings.filter(p => p.type === 'class');
        ...
        allTeachers.forEach((teacher) => {
            ...
            doc.setFontSize(9); doc.setTextColor(0); doc.setFont("helvetica", "bold"); doc.text(`${APP_CONFIG.shortName} - ${teacher.length > 20 ? teacher.substring(0, 18) + "..." : teacher}`, x + 2, y + 5);
        ...
        const doc = new jsPDF('l', 'mm', 'a4'); doc.setFontSize(16); doc.text(`${APP_CONFIG.shortName} Timetable - ${filterVal}`, 14, 18);
